...
def read_files(num_nodes, trace):
    ...
    for i in range(num_nodes):
        tcp_file = "../result/{}/result_{}_{}.txt".format(num_nodes, trace, i)
        output = read_tcp_file(tcp_file)
    ...
    total, num_elements = get_mean_fct_oct_ratio(tcp_out, 6)
    stats['tcp'] = {}
    stats['tcp']['mean'] = [ np.mean(total[j]) for j in range(len(total))]
    stats['tcp']['std'] =  [ np.percentile(total[j], 99) - np.mean(total[j]) for j in range(len(total))]
